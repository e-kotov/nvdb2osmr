...
`split_by = "county"`, the converter can build a global endpoint-node
dictionary to keep node IDs consistent across split boundaries.

- `global_node_prepass = "auto"` (default): enabled for split runs,
  disabled for `split_by = "none"`.
- `global_node_prepass = "on"`: force the prepass for split runs.
- `global_node_prepass = "off"`: disable the prepass.

This is recommended for large split runs where cross-boundary routing
