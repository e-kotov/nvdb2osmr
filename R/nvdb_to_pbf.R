...
#' @export
nvdb_to_pbf <- function(input_path, output_pbf, municipality_codes = NULL, 
  ...
  if (!requireNamespace("mirai", quietly = TRUE)) {
    stop("Package 'mirai' is required for parallel processing. Please install it.")
  }
  
  # --- 0. RESOLVE SOURCE (GDB/GPKG -> GeoParquet if needed) ---
  source_info <- resolve_source(input_path, use_geoparquet, output_pbf, verbose = TRUE)
  ...
  if (presplit) {
    ...
    cli::cli_inform("Largest municipality: {codes[1]} ({format(counts$n[1], big.mark = ',')} segments)")
    ...
    cli::cli_inform("Pre-split {length(source_files)}/{length(codes)} municipalities")
  }
  ...
  mirai::everywhere({ library(nvdb2osmr) })
  ...
  process_municipality <- function(cfg, main_gdb) {
    ...
    chunk_file <- tempfile(pattern = paste0("nvdb_", code, "_"), fileext = ".osm.pbf")
    ...
    tryCatch({
      ...
      list(code = code, file = NULL, n_segments = 0, success = FALSE, error = conditionMessage(e))
    })
  }
  ...
  while (length(pending_configs) > 0 && attempt <= max_retries) {
    if (attempt > 1) {
      cli::cli_inform("Retry attempt {attempt}/{max_retries} for {length(pending_configs)} failed municipality{?ies}...")
    }
    ...
    successful_this_round <- results[sapply(results, function(x) is.list(x) && isTRUE(x$success) && !is.null(x$file))]
    failed_this_round <- results[!sapply(results, function(x) is.list(x) && isTRUE(x$success))]
    ...
    if (length(failed_this_round) > 0) {
      failed_codes <- sapply(failed_this_round, function(x) if (is.list(x)) x$code else NA)
      failed_codes <- failed_codes[!is.na(failed_codes)]
      pending_configs <- chunk_configs[sapply(chunk_configs, function(x) x$code %in% failed_codes)]
      
      if (length(pending_configs) > 0 && attempt < max_retries) {
        cli::cli_warn("{length(pending_configs)} municipality{?ies} failed, will retry: {paste(failed_codes, collapse = ', ')}")
  ...
  if (length(pending_configs) > 0) {
    ...
    for (code in failed_codes) {
      all_results <- c(all_results, list(list(code = code, file = NULL, success = FALSE, error = "Max retries exceeded")))
    }
  }
  ...
  # --- 7. REPORT & MERGE ---
  successful <- all_results[sapply(all_results, function(x) is.list(x) && isTRUE(x$success) && !is.null(x$file))]
  failed <- all_results[!sapply(all_results, function(x) is.list(x) && isTRUE(x$success))]
  
  if (length(failed) > 0) {
    failed_codes <- sapply(failed, function(x) if (is.list(x) && !is.null(x$code)) x$code else "unknown")
    cli::cli_warn("{length(failed)} municipality{?ies} failed after {max_retries} attempt{?s}: {paste(failed_codes, collapse = ', ')}")
    for (f in failed) {
      if (is.list(f)) {
  ...
  cli::cli_inform("")
  cli::cli_alert_success("Completed {length(successful)}/{length(codes)} municipalities in {round(as.numeric(elapsed), 1)} minutes")
  ...
  if (length(chunk_files) == 1) {
    ...
    cli::cli_inform("Merging and sorting {length(chunk_files)} chunks to final PBF...")
    ...
    if (requireNamespace("rosmium", quietly = TRUE)) {
      ...
      cmd <- sprintf("osmium sort %s -o %s --overwrite",
  ...
  # Cleanup temp files
  unlink(chunk_files)
  if (!is.null(temp_dir)) unlink(temp_dir, recursive = TRUE)
  
  # Final stats
  if (file.exists(output_pbf)) {
    size_mb <- file.size(output_pbf) / 1e6
    cli::cli_alert_success("Done! Output: {.file {output_pbf}} ({round(size_mb, 2)} MB)")
  } else {
